    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            <li v-if="options.categoryName" class="with-x">{{options.categoryName}}
              <i @click="removeCategory"> x </i>
            </li>
            <li v-if="options.keyword" class="with-x">{{options.keyword}}
              <i @click="removeKeyword"> x </i>
            </li>
            <li v-if="options.trademark" class="with-x">{{options.trademark}}
              <i @click="removeTrademark"> x </i>
            </li>
            <li v-for="(prop, index) in options.props" :key="prop" class="with-x">{{prop}}
          ...
          <div class="goods-list">
            <ul class="yui3-g">
              <li class="yui3-u-1-5" v-for="item in goodsList" :key="item.id">
                <div class="list-wrap">
                  <div class="p-img">
                    <a href="javascript:" target="_blank"><img :src="item.defaultImg" /></a>
                  </div>
                  <div class="price">
                    <strong>
                      <em> ¥ </em>
  ...
  methods: {
    ...
    */
    setOrder(orderFlag) {
      ...
      let flag = this.options.order.split(':')[0]
      let type = this.options.order.split(':')[1]
      // 点击的是当前的排序项，切换排序方式
      if (orderFlag === flag) {
        type = type === 'desc' ? 'asc' : 'desc'
      } else {  // 点击的不是当前排序项,更新flag为指定的值,排序方式为desc
        flag = orderFlag
        type = 'desc'
      }
      // 请求获取商品分类
