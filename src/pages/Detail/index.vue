...
export default {
  ...
  beforeMount() {   // 一般在beforeMount中处理数据，后期可以直接使用
  ...
  methods: {
    ...
    changeChecked(spuSaleAttrValue, spuSaleAttrValueList) {
      // 让所有都变未选中
      // for (let index = 0; index < spuSaleAttrValueList.length; index++) {
      //   spuSaleAttrValueList[index].isChecked = '0'
      // }
    ...
    // 添加购物车按钮点击事件
    async addShopCart() {
      // 发请求让后台给数据库的购物车数据表添加一条数据
      // 根据添加成功还是失败决定要不要跳转到购物车成功页面
